...
void loadIndividual(int idx, variable* buffer, ifstream & pop_ifs, const vector<std::streamoff> & offsets, int problem_size) {
  // 从硬盘加载第 idx 个个体到 buffer
...
Fitness UADE_MT::run()
{
  ...
  for (int i = 0; i < pop_size; ++i)
  {
    ...
    variable* indiv = makeNewIndividual();          // 生成完整个体在内存
    Fitness fit = eval_sol(indiv);                  // 评估适应度
    ...
    for (int target = 0; target < batch_size; target++) {
      ...
      if (children_fitness[target] < pop[base[target]].first)
      {
        ...
        if (nfes < max_num_evaluations) {
          ...
          pop.emplace_back(children_fitness[target], new_idx);
        }
      }
    }
#ifdef CHRONO
...
void UADE_MT::operateCurrentToPBest1BinWithArchive(const vector<pair<Fitness, int>> &pop,
                                      ...
                                      mt19937 &generator,
                                      ifstream& pop_ifs,
                                      const vector<streamoff>& offsets
  )
{
