...
#pragma omp parallel for private(mu_sf, mu_cr, mu_ps, random_selected_period, p_best_ind) schedule(dynamic)
    for (int target = 0; target < batch_size; target++) {
      ...
      operateCurrentToPBest1BinWithArchive(pop, pbests, children[target], base[target], p_best_ind, r1[target], r2[target], pop_sf[target], pop_cr[target], generators[target]);
