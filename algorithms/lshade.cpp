...
#pragma omp parallel for private(random_selected_period, mu_sf, mu_cr, p_best_ind) schedule(dynamic)
    for (int target = 0; target < pop_size; target++) {
      ...
      random_selected_period = rngs[tid]() % memory_size;
      ...
      p_best_ind = sorted_array[rngs[tid]() % p_num];
      operateCurrentToPBest1BinWithArchive(pop, &children[target][0], target, p_best_ind, pop_sf[target], pop_cr[target], archive, arc_ind_count, rngs[tid]);
    }
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < pop_size; i++) {
      int tid = omp_get_thread_num();
      children_fitness[i] = benchmarks[tid]->compute(children[i]);
    }
    ...
    for (int i = 0; i < pop_size; i++) {
      nfes++;
      if (nfes % 100000 == 0) {
        cout << "Function " << function_number << " Evaluation " << nfes << " Best " << bsf_fitness << " Time " << (long long)((chrono::duration<double>(Clock::now() - t_loop_start).count()))  << " s" << endl;
      }
  ...
  cout << "Total loop time: " << (int)(chrono::duration<double>(t_loop_end - t_loop_start).count() * 1000) << " ms" << endl;

  return bsf_fitness - optimum;
}


void LSHADE::operateCurrentToPBest1BinWithArchive(const vector<Individual> &pop, Individual child, int &target, int &p_best_individual, variable &scaling_factor, variable &cross_rate, const vector<Individual> &archive, int &arc_ind_count, mt19937 &rng) {
  int r1, r2;
  ...
  do {
    r2 = rng() % (pop_size + arc_ind_count);
  } while ((r2 == target) || (r2 == r1));

  int random_variable = rng() % problem_size;
  
  if (r2 >= pop_size) {
    r2 -= pop_size;
    for (int i = 0; i < problem_size; i++) {
      if (((double)rng() / rng.max() < cross_rate) || (i == random_variable)) {
